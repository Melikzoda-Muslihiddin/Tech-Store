...
const rows = qs<HTMLDivElement>("#cartRows");
const subtotalEl = qs<HTMLElement>("#subtotal");
const shippingEl = qs<HTMLElement>("#shipping");
const taxEl = qs<HTMLElement>("#tax");
const totalEl = qs<HTMLElement>("#total");
const checkoutBtn = qs<HTMLButtonElement>("#checkoutBtn");

type Line = { product: Product; qty: number };

function row(line: Line) {
...
async function load() {
  ...
  rows.querySelectorAll<HTMLButtonElement>("[data-inc]").forEach((b) => {
    b.onclick = () => {
  ...
  rows.querySelectorAll<HTMLButtonElement>("[data-dec]").forEach((b) => {
    b.onclick = () => {
  ...
  rows.querySelectorAll<HTMLButtonElement>("[data-del]").forEach((b) => {
    b.onclick = () => {
