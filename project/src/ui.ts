...
import { debounce, qs } from "./utils.js";
...
function ensureProfileModal() {
  let dlg = document.querySelector<HTMLDialogElement>("#profileModal");
  if (dlg) return dlg;
...
export function initHeader(opts: HeaderOptions = {}) {
  refreshCartBadge();
  const profileBtn = document.querySelector<HTMLButtonElement | HTMLAnchorElement>(
    "#profileBtn",
  );
  if (profileBtn) {
    (profileBtn as any).onclick = (e: MouseEvent) => {
      e.preventDefault();
      ...
      const name = dlg.querySelector<HTMLElement>("#pmName");
      const email = dlg.querySelector<HTMLElement>("#pmEmail");
      if (name) name.textContent = user.name;
      if (email) email.textContent = user.email;

      dlg.querySelector<HTMLButtonElement>("#pmClose")!.onclick = () => dlg!.close();
      dlg.querySelector<HTMLButtonElement>("#pmLogout")!.onclick = () => {
        setAuthUser(null);
        dlg!.close();
  ...
  const search = document.querySelector<HTMLInputElement>("#headerSearch");
...
export function refreshCartBadge() {
  const cartBadge = document.querySelector<HTMLSpanElement>("#cartCount");
  if (!cartBadge) return;
  const n = cartCount();
