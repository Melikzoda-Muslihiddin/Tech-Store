...
const mainImg = qs<HTMLImageElement>("#mainImg");
const thumbs = qs<HTMLDivElement>("#thumbs");
const titleEl = qs<HTMLElement>("#title");
const priceEl = qs<HTMLElement>("#price");
const descEl = qs<HTMLElement>("#desc");
const addBtn = qs<HTMLButtonElement>("#addBtn");
const buyBtn = qs<HTMLButtonElement>("#buyBtn");
const similar = qs<HTMLDivElement>("#similar");
const crumb = document.querySelector<HTMLElement>("#titleCrumb");
const colorsWrap = document.querySelector<HTMLDivElement>("#colors");

const DEFAULT_COLORS = ["#111827", "#2563EB", "#10B981", "#F59E0B", "#EF4444"];

function renderThumb(src: string, active = false) {
...
function similarCard(p: Product) {
  return `
...
async function load() {
  ...
  if (!idStr) throw new Error("Missing id param");
  const id = idStr;
  ...
  if (colorsWrap) {
    ...
    colorsWrap.innerHTML = colors
      .map(
        (c, idx) =>
          `<button data-color="${c}" title="${c}" class="w-9 h-9 rounded-full border ${idx === 0 ? "border-blue-600" : "border-slate-200"}" style="background:${c}"></button>`,
      )
      .join("");

    colorsWrap.querySelectorAll<HTMLButtonElement>("[data-color]").forEach((b) => {
      b.onclick = () => {
        colorsWrap
          .querySelectorAll("button")
  ...
  similar.querySelectorAll<HTMLButtonElement>("[data-open]").forEach((btn) => {
    btn.onclick = () => {
