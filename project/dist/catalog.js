...
function card(p) {
  ...
  <div class="rounded-2xl bg-white border border-slate-100 p-4 hover:shadow-sm transition">
    ...
    <div class="mt-2 flex items-center justify-between">
      <div class="text-lg font-bold">${money(p.price ?? 0)}</div>
      <span class="text-[10px] px-2 py-1 rounded-lg bg-emerald-50 text-emerald-700 ${badge ? "" : "hidden"}">${badge}</span>
    </div>

    <div class="mt-3 grid grid-cols-2 gap-2">
      <button data-open="${p.id}" class="rounded-xl bg-slate-900 text-white py-2 text-xs font-semibold">View</button>
      <button data-add="${p.id}" class="rounded-xl bg-blue-600 text-white py-2 text-xs font-semibold">Add to Cart</button>
    </div>
  </div>`;
}
...
async function load() {
    ...
    if (query) {
        filtered = filtered.filter((p) => String(p.title ?? "")
            .toLowerCase()
            .includes(query));
    }
