...
const viewAllBtn = document.querySelector("#viewAllCategories");
const popularWrap = document.querySelector("#popular");
const CATEGORIES = [
    {
        key: "gaming",
        title: "Gaming",
...
function renderCategories() {
    ...
    categoriesWrap.querySelectorAll("[data-cat]").forEach((b) => {
        b.onclick = () => {
    ...
    if (viewAllBtn)
        viewAllBtn.textContent = showAllCats ? "Hide" : "View all";
}
...
async function loadPopular() {
    ...
    popularWrap.innerHTML = Array.from({ length: 4 })
        .map(() => `<div class="rounded-2xl bg-white border border-slate-100 p-4 animate-pulse"><div class="h-40 bg-slate-100 rounded-xl"></div><div class="mt-4 h-4 bg-slate-100 rounded"></div><div class="mt-2 h-4 bg-slate-100 rounded w-2/3"></div></div>`)
        .join("");
    ...
    popularWrap.querySelectorAll("[data-open]").forEach((b) => {
        b.onclick = () => {
            location.href = `productDetails.html?id=${encodeURIComponent(b.dataset.open ?? "")}`;
        };
    });
    popularWrap.querySelectorAll("[data-add]").forEach((b) => {
        b.onclick = () => {
